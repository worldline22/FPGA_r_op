...
struct SInstance
{
    ...
    SInstance(const SInstance& other)
        : fixed(other.fixed),
        Lib(other.Lib),
        id(other.id),
        ...
        numMov(other.numMov) {
            for (auto* pin : other.inpins) {
                if (pin) {
                    inpins.push_back(new SPin(*pin));  // deep copy
                    std::cout << "0" << std::endl;
                }
            }
            for (auto* pin : other.outpins) {
                if (pin) {
                    outpins.push_back(new SPin(*pin));  // deep copy
                }
            }
        }
    ...
    SInstance() {
        ...
        Lib = 0;
        id = 0;
        baseLocation = std::make_tuple(0, 0, 0);
...
struct SNet
{
    ...
    // SNet(const SNet& other)
    //     : id(other.id),
    //     clock(other.clock),
    //     inpin(other.inpin),
    //     outpins(other.outpins),
    //     BBox_L(other.BBox_L),
    //     BBox_R(other.BBox_R),
    //     BBox_U(other.BBox_U),
    //     BBox_D(other.BBox_D) {
